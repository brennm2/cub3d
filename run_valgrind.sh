...
VALID_DIR="assets/map_files/valid"
INVALID_DIR="assets/map_files/invalid"
...
run_valgrind() {
    ...
    if echo "$output" | grep -q "All heap blocks were freed -- no leaks are possible"; then
        ...
        echo "Arquivo: $map_file" >> "$LOG_FILE"
...
run_valid() {
    ...
    if [ -z "$output" ]; then
        ...
        echo -e "\e[31mMapa com falhas!\e[0m" # Mensagem em vermelho se houver saída
        echo "Mapa válido com falhas: $map_file" >> "$LOG_FILE"
        echo "$output" >> "$LOG_FILE"
    fi
}
...
run_invalid() {
    ...
    if [ "$line_count" -le 2 ]; then
        ...
        echo "Mapa inválido com erros: $map_file" >> "$LOG_FILE"
        echo "$output" >> "$LOG_FILE"
    fi
}
